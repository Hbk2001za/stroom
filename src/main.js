...
ipcMain.handle('update-tools', async () => {
  ...
  commands.push(`${pipxCmd} upgrade demucs || ${pipxCmd} install demucs --force`);
  ...
  for (const cmd of commands) {
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        exec(cmd, { timeout: 1200000, maxBuffer: 1024 * 1024 * 50, env: process.env, windowsHide: true }, (err, stdout, stderr) => {
